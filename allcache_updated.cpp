...
#include "pin_cache.H"
FILE *TraceFile = fopen("bench_3_traces.txt","a");
//std::ofstream TraceFile("ptrace.txt");
namespace ITLB
{
...
static VOID Ul2Access(ADDRINT addr, UINT32 size, CACHE_BASE::ACCESS_TYPE accessType)
{
    ...
    if (!ul2Hit)
    {
    	...
    	static uint64_t start = rdtsc();
    	uint64_t end = rdtsc();
    	uint64_t scale_factor = 320;
    	const BOOL ul3hit = ul3.Access(addr, size, accessType);
    	uint64_t mask = 0x0003ffffffff;
    	if(!ul3hit)
    	{
	    if(accessType == 0)
	    	fprintf(TraceFile,"%p READ %lu\n",(ADDRINT*)(addr & mask),(end-start)/scale_factor);
	    else
	    	fprintf(TraceFile,"%p WRITE %lu\n",(ADDRINT*)(addr & mask),(end-start)/scale_factor);
	}
    }
}
